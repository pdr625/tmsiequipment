...
// não tem de viajar com uma lista de preços. A fonte é tmsi.profiles.full_name
// — o user_metadata do GoTrue não o tem (medido). RLS profiles_self limita à
// própria linha; o `.eq(user_id)` é preciso porque um admin veria todas.
async function nomeDeQuemGera(
  supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>,
  userId: string,
): Promise<string> {
...
export async function GET(request: NextRequest) {
  ...
  // Lidos à parte de v_products (tabela + RLS, sem compute_price: barato) e
  // cruzados em memória. Acrescentar colunas à vista seria migração.
  ...
  if (canReadCosts) {
    ...
    const ordenadas = [...rows].sort(
      (a, b) =>
        (meta.get(a.product_id)?.category_id ?? '\uffff').localeCompare(
          meta.get(b.product_id)?.category_id ?? '\uffff',
        ) ||
    ...
    const buffer = await buildXlsx({
      ...
      generatedBy: await nomeDeQuemGera(supabase, user.id),
      generatedAt,
      ...
      rows: ordenadas.map((r) => [
        r.product_id,
        meta.get(r.product_id)?.name ?? '',
        r.branch_id,
        meta.get(r.product_id)?.status ?? '',
        r.currency,
        r.total_cost_eur,
        r.margin,
        r.min_price,
