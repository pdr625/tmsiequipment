...
DuckDNS: point `tmsiequipment.duckdns.org` at the VPS public IP (fixed IP → set once in
...
mkdir -p /opt/tmsiequipment/deploy/supabase
cp -r supabase/docker/* /opt/tmsiequipment/deploy/supabase/
cp supabase/docker/.env.example /opt/tmsiequipment/deploy/supabase/.env
cd /opt/tmsiequipment/deploy/supabase
```
...
| `SITE_URL` | `https://tmsiequipment.duckdns.org` |
| `API_EXTERNAL_URL` | `https://tmsiequipment.duckdns.org/supabase` |
| `SUPABASE_PUBLIC_URL` | same as above |
| `ADDITIONAL_REDIRECT_URLS` | `https://tmsiequipment.duckdns.org/**` |
...
docker exec -i supabase-db psql -U postgres -d postgres \
  < /opt/tmsiequipment/supabase/migrations/0001_initial_schema.sql
# fictitious test data (pilot only):
docker exec -i supabase-db psql -U postgres -d postgres \
  < /opt/tmsiequipment/supabase/seed/0001_test_data.sql
```
...
One proxy host, `tmsiequipment.duckdns.org`, Let's Encrypt certificate, Force SSL, HTTP/2:
